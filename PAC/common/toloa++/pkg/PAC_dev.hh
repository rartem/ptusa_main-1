...
class operation_manager
    ...
    public:
        ...
        ///
        /// @return - время в секундах системы без активных операций.
        unsigned long get_idle_time_sec();

        /// @brief Время бездействия (нет включенных операций).
        ///
        /// @return - время в миллисекундах системы без активных операций.
...
/// @warning Счётчик реализован как 32-битное целое число и переполняется
/// примерно каждые 49,7 суток (2^32 миллисекунд). Для корректного
/// вычисления прошедшего времени используйте функцию get_delta_millisec().
unsigned int get_millisec();
//-----------------------------------------------------------------------------
/// @brief Получение разности времени в миллисекундах.
///
/// @param time1     - начальное время (значение, возвращённое get_millisec()).
/// @return Разность времени в миллисекундах с учётом возможного переполнения
/// счётчика get_millisec().
